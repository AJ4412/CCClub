...
const Mint = () => {
  ...
  async function connect_wallet() {
    if (Web3.givenProvider) {
      ...
      web3.eth.net.getId().then((result) => {
        ...
        if (result !== 4) {
          swal("Wrong Network Selected. Select Ethemerun Mainnet");
        }
      });
  ...
  async function presalemint_nft() {
    if (Web3.givenProvider) {
      ...
      let res = await fetch("https://6lr8onsqi3.execute-api.us-west-2.amazonaws.com/proof/"+address);
      ...
      try {
        const estemated_Gas = await contract.methods.buy_presale(mintNumber,proof).estimateGas({
            from: address,
        ...
        const result = await contract.methods.buy_presale(mintNumber,proof).send({
          from: address,
          value: web3.utils.toWei(total.toString(), "ether"),
          gas: estemated_Gas,
          maxPriorityFeePerGas: null,
